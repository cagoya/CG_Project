...
void LightingPanel::Draw() {
    ...
    if (ImGui::Begin(PanelName, &IsVisible)) { // 开始一个 ImGui 窗口,允许通过窗口的关闭按钮来改变 IsVisible 的值
        ...
        ImGui::Text("phong");
		ImGui::Separator();
        ImGui::ColorEdit3("ka##3", (float*)&materialRef.ka);
        ImGui::ColorEdit3("kd##3", (float*)&materialRef.kd);
        ImGui::ColorEdit3("ks##3", (float*)&materialRef.ks);
        ImGui::SliderFloat("ns##3", &materialRef.ns, 1.0f, 50.0f);
        ImGui::NewLine();
